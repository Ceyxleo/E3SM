...
! !USES:
  ...
  real(r8), parameter :: MOSART_TINYVALUE = 1.0e-14_r8  ! double precision variable has a significance of about 16 decimal digits
...
!-----------------------------------------------------------------------
  ...
  ! !DESCRIPTION: solve the ODEs with Euler algorithm
    ...
    real(r8) :: myMOSART_TINYVALUE
    character(len=*),parameter :: subname = '(Euler)'
    real(r8) :: tmp1, tmp2
    !------------------

    myMOSART_TINYVALUE = 1.e-6
...
!extraction from subnetwork here from wt
             ...
             do k=1,TUnit%numDT_t(iunit)
                ...
                if (heatflag) then
                if (nt==nt_nliq) then
                    if(TUnit%tlen(iunit) > myMOSART_TINYVALUE) then
          ...
          if(TUnit%mask(iunit) > 0) then
             ...
             if (nt==nt_nliq) then
                 do k=1,TUnit%numDT_r(iunit)                
                    if(TUnit%rlen(iunit) > myMOSART_TINYVALUE) then
			 ...
			 if (nt == nt_nliq) then
             !if(TUnit%rlen(iunit) > myMOSART_TINYVALUE) then  ! if no mainchannel, no lake modeling. TODO
                 localDeltaT = Tctl%DeltaT/Tctl%DLevelH2R
...
!#ifdef INCLUDE_WRM
          ...
          if (sediflag .and. wrmflag) then
             ...
             if(Tres_para%Eff_trapping(iunit)>MOSART_TINYVALUE) then            
             ...
             !if(Tres_para%Eff_trapping_r(iunit)>MOSART_TINYVALUE) then             
...
!  !TRunoff%ehout(iunit,nt) = -CREHT(TUnit%hslp(iunit), TUnit%nh(iunit), TUnit%Gxr(iunit), TRunoff%yh(iunit,nt))
    ...
    if(TRunoff%ehout(iunit,nt) < 0._r8 .and. &
       TRunoff%wh(iunit,nt) + (TRunoff%qsur(iunit,nt) + TRunoff%ehout(iunit,nt)) * theDeltaT < MOSART_TINYVALUE) then
...
!-----------------------------------------------------------------------
  ...
  ! !DESCRIPTION: subnetwork channel routing
    ...
    !   !     !TRunoff%vt(iunit,nt) = CRVRMAN(TUnit%tslp(iunit), TUnit%nt(iunit), TRunoff%rt(iunit,nt))
            ...
            if(TRunoff%wt(iunit,nt) + (TRunoff%etin(iunit,nt) + TRunoff%etout(iunit,nt)) * theDeltaT < MOSART_TINYVALUE) then
            ...
            if(TRunoff%etout(iunit,nt) < -MOSART_TINYVALUE .and. &
               TRunoff%wt(iunit,nt) + (TRunoff%etin(iunit,nt) + TRunoff%etout(iunit,nt)) * theDeltaT < MOSART_TINYVALUE) then
              TRunoff%etout(iunit,nt) = -(TRunoff%etin(iunit,nt) + TRunoff%wt(iunit,nt)/theDeltaT)
            end if
        end if
    end if
...
! check stability
!    if(TRunoff%vt(iunit,nt) < -MOSART_TINYVALUE .or. TRunoff%vt(iunit,nt) > 30) then
    ...
    if(TUnit%rlen(iunit) <= 0._r8) then ! no river network, no channel routing
       ...
       if(TUnit%areaTotal2(iunit)/TUnit%rwidth(iunit)/TUnit%rlen(iunit) > 1e6_r8) then
          ...
          if(nt == nt_nliq) then
              ...
              if(-TRunoff%erout(iunit,nt) > MOSART_TINYVALUE .and. TRunoff%wr(iunit,nt) + &
                 (TRunoff%erlateral(iunit,nt) + TRunoff%erin(iunit,nt) + TRunoff%erout(iunit,nt)) * theDeltaT < MOSART_TINYVALUE) then
...
!       call shr_sys_abort('mosart: ERROR temp_gwl too small')
    ...
    if(temp_gwl < -MOSART_TINYVALUE) then 
       ...
       if(TRunoff%wr(iunit,nt) < MOSART_TINYVALUE) then
          temp_gwl = 0._r8
       else 
          if(TRunoff%wr(iunit,nt)/theDeltaT + temp_dwr + temp_gwl < -MOSART_TINYVALUE) then
...
! check for stability
!    if(TRunoff%vr(iunit,nt) < -MOSART_TINYVALUE .or. TRunoff%vr(iunit,nt) > 30) then
!       write(iulog,*) "Numerical error inRouting_KW, ", iunit,nt,TRunoff%vr(iunit,nt)
!    end if

! check for negative wr
!    if(TRunoff%wr(iunit,nt) > 1._r8 .and. (TRunoff%wr(iunit,nt)/theDeltaT + TRunoff%dwr(iunit,nt))/TRunoff%wr(iunit,nt) < -MOSART_TINYVALUE) then
...
!-----------------------------------------------------------------------
  ...
  ! !DESCRIPTION: classic diffusion wave routing method
    ...
    elseif(TUnit%areaTotal2(iunit)/TUnit%rwidth(iunit)/TUnit%rlen(iunit) > 1e6_r8) then ! skip the channel routing if possible numerical instability
       ...
       if(rtmCTL%mask(iunit) .eq. 3) then 
          ...
          if(nt == nt_nliq) then 
              ...
              if(TRunoff%rslp_energy(iunit) >= MOSART_TINYVALUE) then ! flow is from current channel to downstream
                TRunoff%vr(iunit,nt) = CRVRMAN(TRunoff%rslp_energy(iunit), TUnit%nr(iunit), TRunoff%rr(iunit,nt))
                TRunoff%erout(iunit,nt) = -TRunoff%vr(iunit,nt) * TRunoff%mr(iunit,nt)
                if(TRunoff%erin(iunit,nt)*theDeltaT + TRunoff%wr(iunit,nt) <= MOSART_TINYVALUE) then! much negative inflow from upstream, 
                   TRunoff%vr(iunit,nt) = 0._r8
                   TRunoff%erout(iunit,nt) = 0._r8
                elseif(TRunoff%erout(iunit,nt) <= -MOSART_TINYVALUE .and. TRunoff%wr(iunit,nt) + &
                   (TRunoff%erlateral(iunit,nt) + TRunoff%erin(iunit,nt) + TRunoff%erout(iunit,nt)) * theDeltaT < MOSART_TINYVALUE) then
                   TRunoff%erout(iunit,nt) = -(TRunoff%erlateral(iunit,nt) + TRunoff%erin(iunit,nt) + TRunoff%wr(iunit,nt)*MaxStorageDepleted / theDeltaT)
                   if(TRunoff%mr(iunit,nt) > MOSART_TINYVALUE) then
                      TRunoff%vr(iunit,nt) = -TRunoff%erout(iunit,nt) / TRunoff%mr(iunit,nt)
                   end if
                end if
              elseif(TRunoff%rslp_energy(iunit) <= -MOSART_TINYVALUE) then ! flow is from downstream to current channel
                 ...
                 if(rtmCTL%nUp_dstrm(iunit) > 1) then
                     if(TRunoff%erin_dstrm(iunit,nt)*theDeltaT + TRunoff%wr_dstrm(iunit,nt)/rtmCTL%nUp_dstrm(iunit) <= MOSART_TINYVALUE) then! much negative inflow from upstream,
                         TRunoff%vr(iunit,nt) = 0._r8
                         TRunoff%erout(iunit,nt) = 0._r8
                     elseif(TRunoff%erout(iunit,nt) >= MOSART_TINYVALUE .and. TRunoff%wr_dstrm(iunit,nt)/rtmCTL%nUp_dstrm(iunit)- TRunoff%erout(iunit,nt) * theDeltaT < MOSART_TINYVALUE) then
                     ...
                     if(TRunoff%erin_dstrm(iunit,nt)*theDeltaT + TRunoff%wr_dstrm(iunit,nt) <= MOSART_TINYVALUE) then! much negative inflow from upstream,
                         TRunoff%vr(iunit,nt) = 0._r8
                         TRunoff%erout(iunit,nt) = 0._r8
                     elseif(TRunoff%erout(iunit,nt) >= MOSART_TINYVALUE .and. TRunoff%wr_dstrm(iunit,nt) &
                       - TRunoff%erout(iunit,nt) * theDeltaT < MOSART_TINYVALUE) then
            ...
            if(TRunoff%erout(iunit,nt_nliq) <= -MOSART_TINYVALUE) then ! flow is from current channel to downstream
              TRunoff%erout(iunit,nt) = TRunoff%conc_r(iunit,nt) * TRunoff%erout(iunit,nt_nliq)
              if(TRunoff%erin(iunit,nt)*theDeltaT + TRunoff%wr(iunit,nt) <= MOSART_TINYVALUE) then! much negative inflow from upstream, 
                 TRunoff%erout(iunit,nt) = 0._r8
              elseif(TRunoff%erout(iunit,nt) <= -MOSART_TINYVALUE .and. TRunoff%wr(iunit,nt) + &
            ...
            elseif(TRunoff%erout(iunit,nt_nliq) >= MOSART_TINYVALUE) then ! flow is from downstream to current channel
              TRunoff%erout(iunit,nt) = 0._r8
            else
              TRunoff%erout(iunit,nt) = 0._r8
            end if
    ...
    if(TRunoff%erin(iunit,nt) < -MOSART_TINYVALUE .and. TRunoff%erout(iunit,nt) < -MOSART_TINYVALUE) then
...
!    end if
 ...
 !check for negative wr
   ! if((TRunoff%wr(iunit,nt)/theDeltaT + TRunoff%dwr(iunit,nt))/TRunoff%wr(iunit,nt) < -MOSART_TINYVALUE) then
...
!          if(nt == nt_nliq) then
              ...
              elseif(TRunoff%rslp_energy(iunit) <= -MOSART_TINYVALUE) then ! flow is from downstream to current channel
                 ...
                 if(rtmCTL%nUp_dstrm(iunit) > 1) then
                     if( TUnit%ocn_rof_coupling_ID(iunit) .ne. 1 .and. TRunoff%erin_dstrm(iunit,nt)*theDeltaT + TRunoff%wr_dstrm(iunit,nt)/rtmCTL%nUp_dstrm(iunit) <= MOSART_TINYVALUE) then! much negative inflow from upstream,
                         TRunoff%vr(iunit,nt) = 0._r8
                         TRunoff%erout(iunit,nt) = 0._r8
                     elseif( TUnit%ocn_rof_coupling_ID(iunit) .ne. 1 .and. TRunoff%erout(iunit,nt) >= MOSART_TINYVALUE .and. TRunoff%wr_dstrm(iunit,nt)/rtmCTL%nUp_dstrm(iunit)- TRunoff%erout(iunit,nt) * theDeltaT < MOSART_TINYVALUE) then
                         TRunoff%erout(iunit,nt) = TRunoff%wr_dstrm(iunit,nt)*MaxStorageDepleted / theDeltaT / rtmCTL%nUp_dstrm(iunit)
                         if(TRunoff%mr(iunit,nt) > MOSART_TINYVALUE) then
                            TRunoff%vr(iunit,nt) = -TRunoff%erout(iunit,nt) / TRunoff%mr(iunit,nt)
                         end if
                     ...
                     if( TUnit%ocn_rof_coupling_ID(iunit) .ne. 1 .and. TRunoff%erin_dstrm(iunit,nt)*theDeltaT + TRunoff%wr_dstrm(iunit,nt) <= MOSART_TINYVALUE) then! much negative inflow from upstream,
                        TRunoff%vr(iunit,nt) = 0._r8
                        TRunoff%erout(iunit,nt) = 0._r8
                     elseif( TUnit%ocn_rof_coupling_ID(iunit) .ne. 1 .and. TRunoff%erout(iunit,nt) >= MOSART_TINYVALUE .and. TRunoff%wr_dstrm(iunit,nt) &
                       - TRunoff%erout(iunit,nt) * theDeltaT < MOSART_TINYVALUE) then
                        TRunoff%erout(iunit,nt) = TRunoff%wr_dstrm(iunit,nt)*MaxStorageDepleted / theDeltaT
                        if(TRunoff%mr(iunit,nt) > MOSART_TINYVALUE) then
                           TRunoff%vr(iunit,nt) = -TRunoff%erout(iunit,nt) / TRunoff%mr(iunit,nt)
  ...
  ! !DESCRIPTION: update the state variables in subnetwork channel
    ...
    if(nt == nt_nliq) then
       if(TUnit%tlen(iunit) > 0._r8 .and. TRunoff%wt(iunit,nt) > 0._r8) then
        ...
        if(TRunoff%wt(iunit,nt_nliq) >= MOSART_TINYVALUE .and. TRunoff%wt(iunit,nt) >= MOSART_TINYVALUE) then
  ...
  ! !DESCRIPTION: update the state variables in main channel
    ...
    if(nt == nt_nliq) then
       if(TUnit%rlen(iunit) > 0._r8 .and. TRunoff%wr(iunit,nt) > 0._r8) then
        ...
        if(TRunoff%wr(iunit,nt_nliq) >= MOSART_TINYVALUE .and. TRunoff%wr(iunit,nt) >= MOSART_TINYVALUE) then
...
!-----------------------------------------------------------------------
  ...
  ! part 3 is a rectagular with the width rwid0
    ...
    SLOPE1 = SLOPE1def
    if(mr_ <= MOSART_TINYVALUE) then
       hr_ = 0._r8
    else
       if(mr_ - rdepth_*rwidth_ <= MOSART_TINYVALUE) then ! not flooded
          hr_ = mr_/rwidth_
       else ! if flooded, the find out the equivalent depth
          if(mr_ > rdepth_*rwidth_ + (rwidth_ + rwidth0_)*SLOPE1*((rwidth0_-rwidth_)/2._r8)/2._r8 + MOSART_TINYVALUE) then
...
!-----------------------------------------------------------------------
  ...
  ! part 3 is a rectagular with the width rwid0
    ...
    if(hr_ < MOSART_TINYVALUE) then
       pr_ = 0._r8
    else
       if(hr_ <= rdepth_ + MOSART_TINYVALUE) then ! not flooded
          pr_ = rwidth_ + 2._r8*hr_
       else
          if(hr_ > rdepth_ + ((rwidth0_-rwidth_)/2._r8)*SLOPE1 + MOSART_TINYVALUE) then
